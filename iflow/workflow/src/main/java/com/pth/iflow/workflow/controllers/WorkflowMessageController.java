...
public class WorkflowMessageController {
  ...
  public ResponseEntity<WorkflowMessageListEdo> readWorkflow(@PathVariable(required = true) final Long userid, @PathVariable(required = true) final Long lastid, @PathVariable(required = false) Integer status, final HttpServletRequest request, @RequestHeader(TokenVerficationHandlerInterceptor.IFLOW_TOKENID_HEADER_KEY) final String headerTokenId) throws Exception {

    status = status == null ? 0 : status;

    final List<WorkflowMessage> messageList = this.workflowMessageService.getListForUser(userid, lastid, status, headerTokenId);

    return ControllerHelper.createResponseEntity(request,
                                                 new WorkflowMessageListEdo(WorkflowModelEdoMapper.toWorkflowMessageEdoList(messageList)),
                                                 HttpStatus.OK);
  }
