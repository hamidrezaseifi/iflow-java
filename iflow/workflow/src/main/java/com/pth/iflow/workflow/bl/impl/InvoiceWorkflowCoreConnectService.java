...
@Service
public class InvoiceWorkflowCoreConnectService implements IInvoiceWorkflowDataService {
  ...
  public List<InvoiceWorkflow> search(final WorkflowSearchFilter workflowSearchFilter, final String token)
    ...
    final InvoiceWorkflowListEdo edoList = this.restTemplate.callRestPost(
        this.moduleAccessConfig.generateCoreUrl(IflowRestPaths.CoreModule.SEARCH_INVOICEWORKFLOW()), token, EModule.CORE,
        WorkflowModelEdoMapper.toEdo(workflowSearchFilter), InvoiceWorkflowListEdo.class, true);
