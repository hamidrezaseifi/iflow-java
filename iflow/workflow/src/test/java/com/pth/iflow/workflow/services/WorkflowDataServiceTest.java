...
public class WorkflowDataServiceTest extends TestDataProducer {
  ...
  public void testSearchWorkflow() throws Exception {
    ...
    when(this.restTemplate
        .callRestPost(any(URI.class), any(EModule.class), any(WorkflowSearchFilterEdo.class),
            eq(WorkflowListEdo.class), any(String.class), any(boolean.class))).thenReturn(edoList);

    final List<Workflow> resWorkflowList = this.workflowSearchService.search(filter, this.getValidAuthentiocation());
