...
public class WorkflowTypeController {
  ...
  public ResponseEntity<WorkflowTypeListEdo> readWorkflowList(@RequestBody final Set<String> idList,
      final HttpServletRequest request) throws Exception {

    final Set<WorkflowType> modelList = this.workflowTypeService.getListByIdentityList(idList);

    return ControllerHelper.createResponseEntity(request, new WorkflowTypeListEdo(CoreModelEdoMapper.toWorkflowTypeEdoSet(modelList)),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<WorkflowTypeListEdo> readWorkflowListByCompany(@PathVariable final Long id, final HttpServletRequest request)
    ...
    final Set<WorkflowType> modelList = this.workflowTypeService.getListByIdCompanyId(id);
