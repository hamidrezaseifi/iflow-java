...
public class DepartmentGroupControllerTest extends TestDataProducer {
  ...
  public void testReadDepartmentGroupById() throws Exception {
    ...
    this.mockMvc
        .perform(MockMvcRequestBuilders
            .get(IflowRestPaths.CoreModule.DEPARTMENTGRPUP_READ_BY_IDENTITY, model.getIdentity())
  ...
  @Test
  public void testSaveDepartmentGroupBy() throws Exception {

    final DepartmentGroupEntity model = this.getTestDepartmentGroup();
    final DepartmentGroupEdo modelEdo = getTestDepartmentGroupEdo();
    when(this.departmentGroupService.save(any(DepartmentGroupEntity.class))).thenReturn(model);
    when(this.departmentGroupService.fromEdo(any(DepartmentGroupEdo.class))).thenReturn(model);
    when(this.departmentGroupService.toEdo(any(DepartmentGroupEntity.class))).thenReturn(modelEdo);

    final String conetntAsXmlString = this.xmlConverter.getObjectMapper().writeValueAsString(modelEdo);
    final String resultAsXmlString = this.xmlConverter.getObjectMapper().writeValueAsString(modelEdo);

    this.mockMvc
        .perform(MockMvcRequestBuilders
            .post(IflowRestPaths.CoreModule.DEPARTMENTGRPUP_SAVE)
            .header(XmlRestConfig.REQUEST_HEADER_IFLOW_CLIENT_ID, this.innerModulesRequestHeaderValue)
    ...
    verify(this.departmentGroupService, times(1)).save(any(DepartmentGroupEntity.class));
    verify(this.departmentGroupService, times(1)).fromEdo(any(DepartmentGroupEdo.class));
    verify(this.departmentGroupService, times(1)).toEdo(any(DepartmentGroupEntity.class));

  }

  @Test
  public void testDeleteDepartmentGroupBy() throws Exception {
    ...
    this.mockMvc
        .perform(MockMvcRequestBuilders
            .post(IflowRestPaths.CoreModule.DEPARTMENTGRPUP_DELETE)
            .header(XmlRestConfig.REQUEST_HEADER_IFLOW_CLIENT_ID, this.innerModulesRequestHeaderValue)
            .content(conetntAsXmlString)
            .contentType(MediaType.APPLICATION_XML_VALUE))
        .andExpect(status().isOk());

    verify(this.departmentGroupService, times(1)).delete(any(DepartmentGroupEntity.class));
    verify(this.departmentGroupService, times(1)).fromEdo(any(DepartmentGroupEdo.class));
  ...
  public void testReadDepartmentGroupList() throws Exception {
    ...
    this.mockMvc
        .perform(MockMvcRequestBuilders
            .post(IflowRestPaths.CoreModule.DEPARTMENTGRPUP_READ_LIST)
