...
public class OcrResultsDeserializer extends StdDeserializer<OcrResults> {
      ...
      if (jsonField.getKey().toLowerCase().equals("div")) {
        ...
        if (parRootNode.isArray()) {
          ...
          for (int i = 0; i < nodes.size(); i++) {
            ...
            final OcrResultArea ocrArea = this.extractOcrResultArea(node);

            ocrPage.addArea(ocrArea);
          }
        }
        else {
          final OcrResultArea ocrArea = this.extractOcrResultArea(parRootNode);
          ocrPage.addArea(ocrArea);
        }
  ...
  private OcrResultArea extractOcrResultArea(final JsonNode rootNode) {

    final OcrResultArea ocrArea = new OcrResultArea();
      ...
      if (jsonField.getKey().toLowerCase().equals("p")) {
        ...
        if (parRootNode.isArray()) {
          ...
          for (int i = 0; i < nodes.size(); i++) {
            ...
            final OcrResultPar ocrPar = this.extractOcrResultPar(node);

            ocrArea.addPar(ocrPar);
          }
        }
        else {
          final OcrResultPar ocrPar = this.extractOcrResultPar(parRootNode);
          ocrArea.addPar(ocrPar);
        }
  ...
  private OcrResultPar extractOcrResultPar(final JsonNode rootNode) {

    final OcrResultPar ocrPar = new OcrResultPar();
