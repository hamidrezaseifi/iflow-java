...
public class GuiSecurityConfigurations extends WebSecurityConfigurerAdapter implements IGuiConfiguration {
  ...
  protected void configure(final HttpSecurity http) throws Exception {
    ...
    http.authorizeRequests().antMatchers("/auth/testloginmsg").permitAll().antMatchers("/images/*").permitAll().antMatchers("/js/*")
        ...
        .permitAll().antMatchers(LOGIN_PROCESSING_URL).permitAll().antMatchers(LOGIN_URL).permitAll().antMatchers("/admin/**")
        .hasAnyRole("ADMIN", "COMPANY_ADMIN").antMatchers("/admin/user/**", "/admin/data/**", "/admin/data/user/**")
        .hasAnyRole("ADMIN", "COMPANY_ADMIN").antMatchers("/**").authenticated().and();
